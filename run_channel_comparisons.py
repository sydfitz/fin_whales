...
fig, axs = plt.subplots(nrows=9, ncols=1, figsize=(14, 30), sharex=False)

for i, result in enumerate(top3):
    base = i * 3  # offset for each match's 3 rows
    ...
    # --- Plot 2: Segment ---
    axs[base + 1].plot(np.arange(len(segment)) / fps, segment, color='green')
    axs[base + 1].set_title(f'Match {i+1}: Segment from {signal_name}')
    axs[base + 1].set_ylabel('Amplitude')
    axs[base + 1].grid(True)

    # --- Plot 3: Correlation ---
    axs[base + 2].plot(lags / fps, corr, color='purple')
    axs[base + 2].axvline(best_lag_seconds, color='red', linestyle='--', label=f'Best Match @ {best_lag_seconds:.2f}s')
    axs[base + 2].set_title(f'Match {i+1}: Cross-Correlation (Confidence: {confidence:.2f})')
    axs[base + 2].set_xlabel('Lag (seconds)')
    axs[base + 2].set_ylabel('Corr')
    axs[base + 2].legend()
    axs[base + 2].grid(True)
